					...
					@foreach($data as $key=>$val)
						...
						<div class="col-md-3 resent-grid recommended-grid" style="width: 20%">
							<div class="resent-grid-img recommended-grid-img">
								<a href="{{url('films/player',[base64_encode($val->movies_url)])}}">
							...
							<div class="resent-grid-info recommended-grid-info video-info-grid">
								<h5><a href="{{url('films/player',[base64_encode($val->movies_url)])}}" class="title">{{(session()->get('lan')=='en')?$val->en_name:$val->name}}</a></h5>
								<ul>
